...
const CONFIG = {
  ...
  adminEmail: process.env.ADMIN_EMAIL || 'admin@acucaradas.com',
...
function initialize() {
  if (CONFIG.redisEnabled) {
    try {
      ...
      console.log('Brute force protection initialized with Redis storage');
    } catch (error) {
      console.error('Failed to initialize Redis client:', error);
      logger.error('Failed to initialize Redis for brute force protection', { error: error.message });
      console.log('Falling back to memory storage for brute force protection');
    }
  } else {
    console.log('Brute force protection initialized with memory storage');
  }
...
function cleanupMemoryStore() {
  ...
  console.log(`Cleaned up ${count} expired entries from memory store`);
}
...
function loginProtection(options = {}) {
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      console.error('Error in brute force protection middleware:', error);
      logger.error('Brute force protection error', { error: error.message, username, ip });
      next(error);
    }
  };
}
...
function apiRateLimiter(options = {}) {
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      console.error('Error in API rate limiter middleware:', error);
      logger.error('API rate limiter error', { error: error.message, ip, endpoint });
      next(error);
    }
  };
}
