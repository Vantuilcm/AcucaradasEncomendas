...
interface ReviewListItemProps {
  ...
  index: number;
  onLikeReview?: (reviewId: string) => void;
  highlightedReviewId: SharedValue<string | null>;
  likeScale: SharedValue<Record<string, number>>;
  isInitialRender: SharedValue<boolean>;
  getEnteringAnimation: (index: number) => any;
  highlightReview: (reviewId: string) => void;
  animateLikeButton: (reviewId: string) => void;
}
...
const ReviewListItem: React.FC<ReviewListItemProps> = ({
  ...
  const animatedStyle = useAnimatedStyle(() => {
    const isHighlighted = highlightedReviewId.value === item.id;
    return {
      backgroundColor: isHighlighted
        ...
        : withTiming('transparent', { duration: 400, easing: Easing.bezier(0.16, 1, 0.3, 1) }),
      transform: [
        {
          scale: isHighlighted
            ? withSequence(
                withTiming(1.03, { duration: 150, easing: Easing.bezier(0.25, 0.1, 0.25, 1) }),
                withTiming(1, { duration: 300, easing: Easing.bezier(0.16, 1, 0.3, 1) })
              )
            : 1,
        },
      ],
      borderRadius: isHighlighted ? withTiming(wp(12), { duration: 300 }) : withTiming(wp(8), { duration: 300 }),
    };
  });

  const likeButtonStyle = useAnimatedStyle(() => {
    const scale = likeScale.value[item.id] || 1;
    return {
      transform: [{ scale }],
    };
  });
    ...
    >
      <View style={styles.reviewHeader}>
        <Animated.Text
          ...
          entering={SlideInRight.delay(isInitialRender.value ? index * 100 + 100 : 0).duration(400)}
        >
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <ReviewListItem
            ...
            getEnteringAnimation={getEnteringAnimation}
            highlightReview={highlightReview}
            animateLikeButton={animateLikeButton}
          />
        )}
