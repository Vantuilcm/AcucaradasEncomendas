...
function checkFirebaseConsistency() {
  ...
  try {
    ...
    // Verificar iOS - primeiro tenta na raiz se for Expo
    let iosConfigPath = 'GoogleService-Info.prod.plist';
    if (!fs.existsSync(iosConfigPath)) {
      iosConfigPath = 'ios/GoogleService-Info.prod.plist';
    }
    
    if (!fs.existsSync(iosConfigPath)) {
       throw new Error(`ENOENT: no such file or directory, open '${iosConfigPath}'`);
    }
...
function checkEASSecrets() {
  ...
  try {
    // Tenta primeiro o novo comando eas env:list
    let output = '';
    try {
      output = execSync('eas env:list production', { encoding: 'utf8' });
    } catch (e) {
      // Fallback para o antigo eas secret:list se o novo falhar
      output = execSync('eas secret:list', { encoding: 'utf8' });
    }
...
function main() {
  ...
  // Se as variáveis faltam localmente mas existem no EAS, é apenas um aviso
  if (missingVars.length > 0) {
    const reallyMissing = missingVars.filter(v => missingSecrets.includes(v));
    const justLocalMissing = missingVars.filter(v => !missingSecrets.includes(v));

    if (reallyMissing.length > 0) {
      reallyMissing.forEach(v => criticalErrors.push(`Variável ausente (Local e EAS): ${v}`));
    }
    
    if (justLocalMissing.length > 0) {
      log(`\nℹ️  Nota: ${justLocalMissing.length} variáveis ausentes localmente, mas encontradas no EAS.`, 'cyan');
    }
  }

  // Outros erros são sempre críticos
  firebaseErrors.forEach(e => criticalErrors.push(e));
  gitignoreErrors.forEach(e => criticalErrors.push(e));
  
  // EAS Secrets que não são variáveis de ambiente mas são obrigatórias
  const otherMissingSecrets = missingSecrets.filter(s => !missingVars.includes(s));
  otherMissingSecrets.forEach(s => criticalErrors.push(`EAS Secret ausente: ${s}`));
  ...
  if (criticalErrors.length > 0) {
    ...
    log('\n✅ Tudo pronto para o build!', 'green');
    if (sensitiveFiles.length > 0) {
      log('⚠️  Atenção: Arquivos sensíveis encontrados, mas não impedem o build.', 'yellow');
    }
    process.exit(0);
  }
}
