...
if (sentryEnabled) {
  console.log('[ci] SENTRY_ENABLED=1, skipping block');
  process.exit(0);
}
...
roots.forEach((root) => {
  ...
  if (fs.existsSync(iosRoot)) walk(iosRoot);
  walk(root);
});

if (pbxprojFiles.length === 0) {
  console.log('[ci] pbxproj not found under known roots');
  process.exit(0);
}
...
pbxprojFiles.forEach((pbxproj) => {
  ...
  console.log('[ci] blocked sentry phases in pbxproj:', pbxproj);
});
