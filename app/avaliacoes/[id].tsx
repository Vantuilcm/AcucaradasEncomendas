...
import { SocialAuthService, SocialAuthResult } from '../../src/services/SocialAuthService';
...
// Opções de ordenação
const opcoesFiltro: { id: ReviewFilters['sortBy']; label: string }[] = [
...
export default function AvaliacoesProduto() {
  ...
  const [produto, setProduto] = useState<{ id: string; nome: string } | null>(null);
  ...
  const socialAuthService = useMemo(() => SocialAuthService.getInstance(), []);
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        ...
        logger.error('Erro ao verificar status de autenticação:', error instanceof Error ? error : new Error(String(error)));
      }
    };
    ...
    async (refresh = false) => {
      try {
        ...
        const reviewsData = await reviewService.getReviews(
          filters,
          refresh ? undefined : lastReview ?? undefined,
        );
        ...
        try {
          ...
          logger.error('Erro ao carregar produto:', produtoError instanceof Error ? produtoError : new Error(String(produtoError)));
        }
        ...
        logger.error('Erro ao carregar avaliações:', err instanceof Error ? err : new Error(String(err)));
      } finally {
        setIsLoading(false);
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook' | 'apple') => {
    try {
      ...
      logger.error(`Erro ao fazer login com ${provider}:`, error instanceof Error ? error : new Error(String(error)));
  ...
  const handleVerifyCode = async () => {
    try {
      ...
      logger.error('Erro ao verificar código 2FA:', error instanceof Error ? error : new Error(String(error)));
  ...
  const handleLikeReview = async (reviewId: string) => {
    try {
      ...
      logger.error('Erro ao curtir avaliação:', err instanceof Error ? err : new Error(String(err)));
