...
export class TwoFactorAuthService {
  ...
  private async generateBackupCodes(): Promise<string[]> {
    ...
    for (let i = 0; i < BACKUP_CODES_COUNT; i++) {
      // Gerar bytes aleatórios usando expo-crypto em vez de expo-random
      const randomBytes = Crypto.getRandomValues(new Uint8Array(BACKUP_CODE_LENGTH / 2));
  ...
  private async hashBackupCode(code: string): Promise<string> {
    ...
    // Criar hash SHA-256 usando expo-crypto
    return await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, cleanCode);
  }
