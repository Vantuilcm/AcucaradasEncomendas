...
import { PermissionsService, Role, Permission } from '../services/PermissionsService';
...
const UserManagementScreen: React.FC = () => {
  ...
  const carregarUsuarios = useCallback(
    async (limpar = false) => {
      try {
        ...
        // Use snapshot.docs to avoid implicit any and align with Firestore typings
        snapshot.docs.forEach(docSnap => {
          listaUsuarios.push({ id: docSnap.id, ...docSnap.data() } as User);
        });
        ...
        logger.error('Erro ao carregar usuários:', error instanceof Error ? error : new Error(String(error)));
  ...
  const buscarUsuario = useCallback(async () => {
    ...
    try {
      ...
      logger.error('Erro ao buscar usuários:', error instanceof Error ? error : new Error(String(error)));
  ...
  useEffect(() => {
    const verificarPermissoes = async () => {
      const temPermissao = hasPermission(Permission.GERENCIAR_USUARIOS);
    ...
    verificarPermissoes();
    carregarUsuarios(true);
  }, [carregarUsuarios, hasPermission, isAdmin, navigation]);

  const mudarPapelUsuario = async (userId: string, novoRole: Role) => {
    try {
      ...
      logger.error('Erro ao alterar papel do usuário:', error instanceof Error ? error : new Error(String(error)));
  ...
  const resetarPermissoes = async (userId: string) => {
    try {
      ...
      logger.error('Erro ao redefinir permissões:', error instanceof Error ? error : new Error(String(error)));
  ...
  const renderItem = ({ item }: { item: User }) => {
    const role = (item.role as Role) || Role.CLIENTE;
  ...
  return (
    <ProtectedRoute requiredPermissions={[Permission.GERENCIAR_USUARIOS]}>
          ...
          <View style={styles.filterContainer}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  setFiltroPapel(Role.ENTREGADOR);
                  setUltimoUsuario(null);
                  setTimeout(() => carregarUsuarios(true), 100);
                }}
              >
