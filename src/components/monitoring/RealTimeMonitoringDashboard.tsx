 ...
 * Componente React que exibe métricas, alertas e status do sistema de busca
...
const RealTimeMonitoringDashboard: React.FC = () => {
  ...
  const connectWebSocket = () => {
    try {
      // Em um ambiente real, você usaria uma URL WebSocket real
      // Para este exemplo, vamos simular a conexão
      const mockWebSocket = {
        send: (data: string) => {
          console.log('Enviando dados via WebSocket:', data);
        },
        close: () => {
      ...
      // Adicionar conexão ao sistema de monitoramento
  ...
  // Gerar métricas mock para demonstração
  ...
  const generateMockAlerts = (): DashboardAlert[] => {
    ...
    if (Math.random() > 0.7) {
      alerts.push({
        ...
        threshold: 500,
        timestamp: Date.now(),
        message: 'Latência de busca acima do limite',
        acknowledged: false,
      });
    }
  ...
  // Gerar status de saúde mock para demonstração
  ...
  const onRefresh = async () => {
    ...
    try {
      ...
      const recentAlerts = searchMonitor.getRecentAlerts(300000); // Últimos 5 minutos
  ...
  // Função para obter cor baseada no nível do alerta
    ...
    >
        ...
        {Object.entries(realTimeData.metrics).map(([metric, data]) => (
          <View key={metric} style={styles.metricCard}>
            ...
            <View style={styles.metricValues}>
              <Text style={styles.metricValue}>Média: {data.avg?.toFixed(2) || 'N/A'}</Text>
          ...
          realTimeData.alerts.map(alert => (
            ...
            >
              ...
              <Text style={styles.alertDetails}>
                Métrica: {alert.metric} | Valor: {alert.value.toFixed(2)} | Limite:{' '}
      ...
      <View style={styles.footer}>
        <Text style={styles.lastUpdate}>
          Última atualização: {new Date(realTimeData.timestamp).toLocaleString()}
        </Text>
      </View>
    </ScrollView>
  );
