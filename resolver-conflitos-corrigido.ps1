...
# NPMConflictSolverAI - Versão Corrigida
...
# Função para exibir mensagens formatadas (com cores seguras do PowerShell)
...
# Garantir que expo-router tenha a mesma versão em dependencies e overrides
if ($packageJson.dependencies."expo-router") {
    $expoRouterVersion = $packageJson.dependencies."expo-router"
    $packageJson.overrides."expo-router" = $expoRouterVersion
    Write-ColorMessage "5. Alinhando versão do expo-router: $expoRouterVersion" "Magenta"
}
...
Write-ColorMessage "✅ Versão do expo-router alinhada entre dependencies e overrides" "Green"
