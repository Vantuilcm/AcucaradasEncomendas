...
async function checkPermission(permission) {
  try {
    ...
    if (typeof Permissions === 'undefined') {
      console.warn('Módulo de permissões não disponível neste ambiente');
      return false;
    }
...
async function secureStore(key, value) {
  try {
    ...
    if (typeof SecureStore === 'undefined') {
      console.warn('SecureStore não disponível neste ambiente');
      return false;
    }
    ...
    console.error(`Erro ao armazenar ${key} de forma segura:`, error);
    return false;
  }
}
...
async function secureRetrieve(key) {
  try {
    ...
    console.error(`Erro ao recuperar ${key} do armazenamento seguro:`, error);
    return null;
  }
}
...
async function isDeviceRooted() {
  try {
    ...
    if (typeof Platform === 'undefined') {
      console.warn('Detecção de root/jailbreak não disponível neste ambiente');
      return false;
    }
    ...
    console.error('Erro ao verificar se o dispositivo está rooteado/jailbroken:', error);
    return false;
  }
}
...
async function isRunningOnEmulator() {
  try {
    // Verifica se estamos em ambiente mobile
    if (typeof Platform === 'undefined') {
      console.warn('Detecção de emulador não disponível neste ambiente');
      return false;
    }
    ...
    console.error('Erro ao verificar se está rodando em um emulador:', error);
    return false;
  }
}
...
async function isBeingDebugged() {
  try {
    // Verifica se estamos em ambiente mobile
    if (typeof Platform === 'undefined') {
      console.warn('Detecção de depuração não disponível neste ambiente');
      return false;
    }
    ...
    console.error('Erro ao verificar se está sendo depurado:', error);
    return false;
  }
}
...
async function preventScreenCapture() {
  try {
    ...
    if (typeof ScreenCapture === 'undefined') {
      console.warn('Prevenção de captura de tela não disponível neste ambiente');
      return false;
    }
    ...
    console.error('Erro ao prevenir captura de tela:', error);
    return false;
  }
}
