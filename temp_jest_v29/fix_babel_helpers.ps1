...
Function Install-Package {
    ...
    while (-not $downloadSuccess -and $retryCount -lt $maxRetries) {
        try {
            ...
            # Use specific version if provided, else just package name (latest)
            if ($version -eq "latest") {
                cmd /c "npm pack $pkgName" | Out-Null
            } else {
    ...
    if ($tgz) {
        ...
        Write-Error "Failed to find tarball for $pkgName (Pattern: $tarNamePattern)"
    }
}

# Fix corrupted @babel/helpers
Install-Package "@babel/helpers" "latest"

Write-Host "ALL DONE - Babel Helpers Fixed"
