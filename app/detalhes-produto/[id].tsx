...
import { View, StyleSheet, ScrollView, Text, TouchableOpacity, KeyboardAvoidingView, Platform, Keyboard } from 'react-native';
...
import { ENTRANCE_TIMING_CONFIG, BUTTON_SPRING_CONFIG, getStaggeredDelay } from '../../src/utils/animations';
import {
  wp,
  hp,
  fontSize,
...
// Tipos para dados simulados do produto
...
const produtosMock: Record<string, ProdutoMock> = {
  '1': {
    id: '1',
...
export default function DetalhesProduto() {
  ...
  // Garantir que o id seja uma string única (expo-router pode retornar string|string[])
  const productId = Array.isArray(id) ? id[0] : (id ?? "");
  const router = useRouter();
  const rootNavigationState = useRootNavigationState();
  const { addItem } = useCart();
  const [isLoading, setIsLoading] = useState(true);
  const [produto, setProduto] = useState<ProdutoMock | null>(null);
  const [novaAvaliacao, setNovaAvaliacao] = useState<{ nota: number; comentario: string; mostrando: boolean }>({
    nota: 0,
    comentario: '',
    mostrando: false,
  });
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const produtoData = produtosMock[productId as string];
  ...
  const enviarAvaliacao = (formData: ReviewFormData) => {
    ...
    const avaliacoes = [...produto.avaliacoes, novaAvaliacaoObj as unknown as AvaliacaoMock];
  ...
  const handleVoiceSearchResult = (text: string) => {
    ...
    safePush(`/produtos?search=${encoded}`);
  };
      ...
      >
        ...
        <View style={styles.avaliacaoActionButtons}>
          ...
          <TouchableOpacity
            ...
            onPress={() => safePush(`/avaliacoes/${produto.id}`)}
          >
  ...
  // Tratamento para id inexistente ou produto não encontrado
  if (!produto) {
    return (
      ...
      >
            ...
            >
              <VoiceSearch
                ...
                onError={(error: unknown) => {
                  logger.error('Erro na busca por voz:', error instanceof Error ? error : new Error(String(error)));
                }}
        ...
        <View style={[styles.loadingContainer, { paddingHorizontal: spacing(16) }]}>
          <Animated.Text style={styles.loadingText} entering={FadeIn.duration(400)}>
            Produto não encontrado.
          </Animated.Text>
          <Text style={{ marginTop: spacing(8), color: '#666' }}>
            Verifique o link ou selecione um produto válido.
          </Text>
          <View style={{ marginTop: spacing(16), width: '100%' }}>
            <Button
              title="Ir para Produtos"
              onPress={() => safePush('/produtos')}
              variant="secondary"
              style={styles.button}
            />
          </View>
        ...
        >
          <Animated.View style={contentAnimatedStyle}>
            ...
            <Animated.View entering={FadeInDown.delay(100).duration(400)}>
              {produto ? <Text style={styles.produtoNome}>{produto.nome}</Text> : null}
              ...
              <View style={styles.ratingContainer}>
                ...
                <Text style={styles.ratingText}>
                  {notaMedia.toFixed(1)} ({produto ? produto.avaliacoes.length : 0}{' '}
                  {produto && produto.avaliacoes.length === 1 ? 'avaliação' : 'avaliações'})
                </Text>
              </View>
        ...
        <Button
          ...
          onPress={async () => {
            ...
            const options = opcoesSelecionadas
              .map(opId => {
                const op = produto.opcoes.find(o => o.id === opId);
                return op ? { id: op.id, name: op.nome, price: op.preco } : null;
              })
              .filter(Boolean) as { id: string; name: string; price: number }[];
            await addItem({
              productId: produto.id,
              producerId: (produto as any).producerId || 'producer_test_123',
              name: produto.nome,
              price: produto.preco,
              quantity: 1,
