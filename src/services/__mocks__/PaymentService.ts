class PaymentService {
  ...
  consultarPagamento = jest.fn().mockImplementation(async (idPagamento: string) => {
  ...
  cancelarPagamento = jest.fn().mockImplementation(async (idPagamento: string) => {
    const pagamento = this.pagamentos.get(idPagamento);
    if (!pagamento) {
      throw new Error('Pagamento não encontrado');
    }
  ...
  reembolsarPagamento = jest.fn().mockImplementation(async (idPagamento: string) => {
    const pagamento = this.pagamentos.get(idPagamento);
    if (!pagamento) {
      throw new Error('Pagamento não encontrado');
    }
