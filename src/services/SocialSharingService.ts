...
export class SocialSharingService {
  ...
  public async shareProductImage(product: Product): Promise<boolean> {
    try {
      ...
      const tempFile = new File(cacheDir, `temp_${product.id}.jpg`);
      await File.downloadFileAsync(imageUrl, tempFile, { idempotent: true });

      // Compartilhar a imagem
      await (Sharing as any).shareAsync(String((tempFile as any).uri), {
        mimeType: 'image/jpeg',
  ...
  public async inviteFriends(message?: string): Promise<boolean> {
    try {
      ...
      loggingService.error('Erro ao convidar amigos', error instanceof Error ? error : undefined);
      return false;
    }
  }
  ...
  public async getWishlist(userId: string): Promise<WishlistItem[]> {
    try {
      ...
      loggingService.error(
        'Erro ao obter lista de desejos',
        error instanceof Error ? error : undefined,
        { userId }
      );
      return [];
  ...
  public async shareWishlist(userId: string, userName: string): Promise<boolean> {
    try {
      ...
      loggingService.error(
        'Erro ao compartilhar lista de desejos',
        error instanceof Error ? error : undefined,
        { userId }
      );
      return false;
  ...
  public async updateWishlistItemNotes(
    ...
    try {
      ...
      loggingService.error(
        'Erro ao atualizar notas do item',
        error instanceof Error ? error : undefined,
        { userId, productId }
      );
      return false;
