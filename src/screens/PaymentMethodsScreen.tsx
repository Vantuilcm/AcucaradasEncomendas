...
export const PaymentMethodsScreen: React.FC = () => {
  ...
  const loadPaymentMethods = async () => {
    try {
      ...
      const [userCards, userPixKeys] = await Promise.all([
        PaymentService.getInstance().getPaymentCards(user!.uid),
        PaymentService.getInstance().getPixKeys(user!.uid),
      ]);
      setCards(userCards);
  ...
  const handleSetDefaultCard = async (cardId: string) => {
    try {
      setLoading(true);
      await PaymentService.getInstance().setDefaultCard(user!.uid, cardId);
      loadPaymentMethods();
  ...
  const handleSetDefaultPixKey = async (pixKeyId: string) => {
    try {
      setLoading(true);
      await PaymentService.getInstance().setDefaultPixKey(user!.uid, pixKeyId);
      loadPaymentMethods();
                ...
                cards.map(card => (
                  <View key={card.id} style={styles.cardItem}>
                    <View style={styles.cardInfo}>
                      <Text variant="titleMedium">{maskCardNumber(card.number)}</Text>
