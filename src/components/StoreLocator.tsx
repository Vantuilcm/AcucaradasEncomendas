...
export const StoreLocator: React.FC<StoreLocatorProps> = ({ onSelectStore, productId }) => {
  ...
  const handleRefresh = async () => {
    try {
      ...
      loggingService.error('Erro ao atualizar lista de lojas', error as Error);
    } finally {
      setRefreshing(false);
    }
  };
    ...
    return (
      ...
      >
        <View style={styles.logoColumn}>
          ...
          {(item.producerPhotoUrl || item.producerName) && item.producerPhotoUrl && (
            <View style={styles.producerBadge}>
              <Image source={{ uri: item.producerPhotoUrl }} style={styles.producerPhoto} />
            </View>
          )}
        </View>
        ...
        <View style={styles.storeInfo}>
          ...
          {(item.producerPhotoUrl || item.producerName) && (
            <View style={styles.producerRow}>
              {item.producerPhotoUrl && (
                <Image source={{ uri: item.producerPhotoUrl }} style={styles.producerInlinePhoto} />
              )}
              {item.producerName && (
                <Text style={styles.producerName}>Produtor: {item.producerName}</Text>
              )}
            </View>
          )}
