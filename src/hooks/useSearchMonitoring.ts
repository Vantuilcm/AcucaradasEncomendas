/**
 * Hook personalizado para integração com o sistema de monitoramento de busca
 ...
 * @param refreshInterval Intervalo de atualização em milissegundos
 * @returns Estado e ações do monitoramento
 */
export const useSearchMonitoring = (
  ...
  const recordSearchLatency = useCallback((latency: number, query: string) => {
    try {
      ...
      console.error('Erro ao registrar latência de busca:', error);
    }
  }, []);
  ...
  const recordZeroResults = useCallback((query: string, filters?: Record<string, any>) => {
    try {
      ...
      console.error('Erro ao registrar busca sem resultados:', error);
    }
  }, []);
...
/**
 * Hook simplificado para registrar métricas de busca
 * @returns Funções para registrar métricas comuns
 */
export const useSearchMetrics = () => {
  ...
  const recordSearchEnd = useCallback((startTime: number, query: string, resultCount: number) => {
    ...
    // Registrar busca sem resultados se aplicável
    if (resultCount === 0) {
 ...
 * @returns Funções para monitorar performance
 */
