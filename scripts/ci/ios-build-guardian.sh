...
# 4. Execução do Build iOS LOCAL (Obrigatório)
echo "🚀 [BUILD] Iniciando EAS Build LOCAL (Perfil: production_v13)..."
eas build --platform ios --profile production_v13 --local --non-interactive 2>&1 | tee build-logs/local-build.log
BUILD_EXIT_CODE=$?

if [ $BUILD_EXIT_CODE -ne 0 ]; then
    echo "❌ [ERROR] Build Local falhou. Verifique os logs em build-logs/local-build.log"
    exit 1
fi
