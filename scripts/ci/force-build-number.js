...
class BuildNumberEnforcer {
  ...
  enforce() {
    ...
    // Se FIXED_BUILD_NUMBER estiver definido, usar ele.
    let finalBN;
    if (process.env.FIXED_BUILD_NUMBER) {
      finalBN = parseInt(process.env.FIXED_BUILD_NUMBER, 10);
      console.log(`🎯 [FIXED] Usando Build Number fixo: ${finalBN}`);
    } else {
      const baseline = Math.max(currentLocal, appleLatest, historyLatest, 914);
      finalBN = baseline + 1;
    }
