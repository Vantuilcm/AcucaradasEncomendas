...
export const OptimizedStateProvider: React.FC<OptimizedStateProviderProps> = ({ children }) => {
  ...
  const actions = useMemo(() => ({
    ...
    persistState: async () => {
      try {
        await cacheService.set('global_state', state, 7 * 24 * 60 * 60 * 1000); // 7 dias
    ...
    hydrateState: async () => {
      try {
        ...
        const cachedState = await cacheService.get<Partial<GlobalState>>('global_state');
        ...
        if (__DEV__) {
          console.error('Erro ao hidratar estado:', error);
        }
      }
    },
  }), [state, cacheService]);
