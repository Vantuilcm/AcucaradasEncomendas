...
import { CourierOrderService } from '../services/CourierOrderService';
...
export function DriverHomeScreen() {
  ...
  const courierOrderService = useMemo(() => CourierOrderService.getInstance(), []);
    ...
    try {
      ...
      if (isDriverLoadForensicEnv) {
        console.error('[DRIVER_LOAD_FORENSIC]', {
          ...
          code: loadError?.code ?? 'sem c├│digo',
        });
      }
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    const loadOrdersOnce = async () => {
      ...
      try {
        ...
        const allOrders = await courierOrderService.getOrders();

        if (!active) return;

        const available = allOrders.filter(
          o => o.status === 'ready' && !o.deliveryDriver && !o.deliveryDriverId
        );
  ...
  }, [user, driver?.id, courierOrderService]);
  ...
  const toggleOnline = async (value: boolean) => {
    ...
    if (!driver?.id) {
      console.warn('[DRIVER_ONLINE_STATUS] Perfil de entregador n├úo encontrado no estado local');
      if (isDriverToggleDiagnosticEnv) {
        Alert.alert(
          'Erro T├®cnico',
          [
            'Fase: toggleAvailability',
            'Mensagem: Perfil de entregador n├úo carregado (driver.id ausente)',
            'C├│digo: sem c├│digo',
          ].join('\n')
        );
      } else {
        Alert.alert('Erro', 'Perfil de entregador n├úo carregado. Puxe a tela para baixo para atualizar.');
      }
      return;
    }
      ...
      if (isDriverToggleDiagnosticEnv) {
        Alert.alert(
          ...
          [
            ...
            `C├│digo: ${toggleError?.code || 'sem c├│digo'}`,
          ].join('\n')
        );
      } else {
        Alert.alert('Erro', 'N├úo foi poss├¡vel alterar seu status. Verifique sua conex├úo.');
      }
    }
  };
  ...
  const handleAcceptOrder = async (order: Order) => {
    ...
    if (!canAcceptOrders(driver.status)) {
      ...
      Alert.alert(
        'Cadastro em an├ílise',
        'Aguarde a aprova├º├úo do seu cadastro.'
      );
      return;
    }

    try {
      // Usa o novo m├®todo at├┤mico para blindar corrida fantasma/concorr├¬ncia
      const acceptedOrder = await courierOrderService.acceptOrderAtomic(order.id, {
        id: driver.id,
        name: driver.name,
        phone: driver.phone,
      ...
      Alert.alert('Sucesso', 'Entrega aceita! V├í at├® a loja para retirar.');
    } catch (error: any) {
      Alert.alert('Ops', error.message || 'N├úo foi poss├¡vel aceitar esta entrega.');
    }
  };

  const handleUpdateStatus = async (orderId: string, status: OrderStatus) => {
    try {
      await courierOrderService.updateOrderStatus(orderId, status);
    } catch (error) {
      Alert.alert('Erro', 'N├úo foi poss├¡vel atualizar o status.');
    }
  };
  ...
  const getStatusBadge = (status: OrderStatus) => {
    switch(status) {
      case 'ready': return <Chip style={{ backgroundColor: '#FFF9C4' }} textStyle={{ color: '#F57F17', fontWeight: 'bold', fontSize: 12 }}>­ƒƒí Aguardando retirada</Chip>;
      case 'delivering': return <Chip style={{ backgroundColor: '#E1BEE7' }} textStyle={{ color: '#6A1B9A', fontWeight: 'bold', fontSize: 12 }}>­ƒƒú Em rota</Chip>;
      case 'delivered': return <Chip style={{ backgroundColor: '#C8E6C9' }} textStyle={{ color: '#2E7D32', fontWeight: 'bold', fontSize: 12 }}>­ƒƒó Entregue</Chip>;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Surface style={[styles.statusHeader, isOnline ? styles.statusHeaderOnline : styles.statusHeaderOffline]} elevation={3}>
          <View style={styles.statusRow}>
            <View style={{ flex: 1, paddingRight: 16 }}>
              <Title style={styles.welcomeText}>Ol├í, {driver?.name?.split(' ')[0] || 'Entregador'}</Title>
              <Text style={isOnline ? styles.statusLabelOnline : styles.statusLabelOffline}>
                {isOnline ? '­ƒƒó Voc├¬ est├í online\nPronto para receber entregas pr├│ximas' : 'ÔÜ¬ Voc├¬ est├í offline\nAtive para come├ºar a receber corridas'}
              </Text>
            </View>
            <Switch 
              value={isOnline} 
            ...
            {activeOrders.map(order => (
              <Card key={order.id} style={[styles.premiumCard, styles.activeCard]}>
                <Card.Content>
                  ...
                  <List.Item
                    title="Origem (Loja)"
                    description="Retirar no endere├ºo da loja"
        ...
        <View style={styles.section}>
          <Title style={styles.sectionTitle}>Corridas Dispon├¡veis ({availableOrders.length})</Title>
          {!isOnline ? (
            <View style={styles.emptyPremiumContainer}>
              <MaterialCommunityIcons name="moped-outline" size={56} color="#ccc" />
              <Text style={styles.emptyPremiumTitle}>Nenhuma entrega dispon├¡vel agora ­ƒøÁ</Text>
              <Text style={styles.emptyPremiumSubtitle}>Fique online que avisaremos novas corridas pr├│ximas.</Text>
            </View>
          ) : availableOrders.length === 0 ? (
            <View style={styles.emptyPremiumContainer}>
              <MaterialCommunityIcons name="radar" size={56} color="#4CAF50" />
              <Text style={styles.emptyPremiumTitle}>Buscando entregas... ­ƒøÁ</Text>
              <Text style={styles.emptyPremiumSubtitle}>Aguarde, estamos buscando as melhores corridas pr├│ximas a voc├¬.</Text>
            </View>
          ) : (
                ...
                <Card.Content>
                  ...
                  <View style={styles.orderPremiumDetails}>
                    <View style={styles.detailRow}>
                      ...
                      <Text style={styles.detailText}>{order.deliveryAddress?.neighborhood || 'Bairro indispon├¡vel'}</Text>
                    </View>
                    <View style={styles.detailRow}>
                      <MaterialCommunityIcons name="bike" size={16} color="#666" />
                      <Text style={styles.detailText}>Entrega pr├│xima</Text>
                    </View>
                  </View>
        ...
        <View style={styles.footer}>
          <Text style={styles.versionText}>
            {Constants.expoConfig?.version ? `v${Constants.expoConfig.version}` : 'Driver'}
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
