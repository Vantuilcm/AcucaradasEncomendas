...
/**
 * Configuração para marcas d'água dinâmicas
 */
export interface WatermarkConfig {
  /** Habilitar marcas d'água dinâmicas em conteúdo sensível */
  enabled: boolean;
  /** Opacidade da marca d'água (0-1) */
  opacity: number;
  /** Rotação da marca d'água em graus */
  rotation: number;
  /** Texto padrão para a marca d'água */
  defaultText: string;
  /** Intervalo em ms para atualizar a marca d'água */
  updateInterval: number;
  /** Habilitar registro de logs */
  enableLogging: boolean;
  /** Telas onde a marca d'água deve ser sempre ativada */
  protectedScreens: string[];
}
...
export const screenshotProtectionConfig: ScreenshotProtectionConfig = {
  ...
  blurContent: true,
  showWarning: true,
  warningText: 'Captura de tela detectada! Por segurança, o conteúdo sensível foi ocultado.',
...
export const watermarkConfig: WatermarkConfig = {
  ...
  opacity: 0.3,
  rotation: -45,
  defaultText: 'CONFIDENCIAL - Açucaradas Encomendas',
