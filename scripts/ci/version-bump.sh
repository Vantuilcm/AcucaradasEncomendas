...
CURRENT_VERSION=$(grep -m 1 '"version":' "$APP_JSON" | cut -d'"' -f4)

if [ "$PLATFORM" == "ios" ]; then
    CURRENT_BUILD=$(grep -m 1 '"buildNumber":' "$APP_JSON" | cut -d'"' -f4)
    ...
    CURRENT_CODE=$(grep -m 1 '"versionCode":' "$APP_JSON" | cut -d' ' -f4 | tr -d ',')
