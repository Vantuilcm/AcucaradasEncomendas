...
export const isUserAdmin = async (userId: string): Promise<boolean> => {
  try {
    ...
    loggingService.error(
      'Erro ao verificar permissões de administrador',
      error instanceof Error ? error : undefined,
      { userId }
    );
    return false;
 ...
 */
export const obterInformacoesUsuario = (): UserInfo | null => {
  return getUserInfo();
};

/**
 * Alias para getUserDetails para compatibilidade
 */
export const obterDetalhesUsuario = async (userId: string): Promise<User | null> => {
  return getUserDetails(userId);
};

/**
 * Alias para isUserAdmin para compatibilidade
 */
export const usuarioEhAdmin = async (userId: string): Promise<boolean> => {
