 ...
 *
 * Este arquivo contém testes para validar o impacto do monitoramento
...
const mockProducts: Product[] = [
  ...
  },
  {
    ...
    ingredientes: ['morango', 'farinha', 'açúcar'],
    peso: 100,
...
export class MonitoringPerformanceTest {
  ...
  async runAllTests(): Promise<PerformanceTestSummary> {
    console.log('🚀 Iniciando testes de performance do monitoramento...');
  ...
  private async setupTestData(): Promise<void> {
    ...
    console.log(`✅ ${mockProducts.length} produtos indexados para teste`);
  }

  /**
   * Testa o impacto do monitoramento na latência de busca
   */
  private async testSearchLatencyImpact(): Promise<void> {
    console.log('⏱️ Testando impacto na latência de busca...');
    ...
    this.testResults.push({
      ...
      minValue: minLatency,
      iterations,
      passed: avgLatency < 50, // Deve ser menor que 50ms em média
      details: {
        description: 'Impacto do monitoramento na latência de busca',
        threshold: '< 50ms média',
        unit: 'ms',
      },
    });
  ...
  /**
   * Testa o impacto do monitoramento no uso de memória
   */
  private async testMemoryUsageImpact(): Promise<void> {
    console.log('💾 Testando impacto no uso de memória...');

    const initialMemory = this.getMemoryUsage();

    // Executar muitas operações de monitoramento
    ...
    this.testResults.push({
      ...
      details: {
        description: 'Impacto do monitoramento no uso de memória',
    ...
    console.log(
      `📊 Uso de memória: inicial ${initialMemory}MB, final ${finalMemory}MB, aumento ${memoryIncrease}MB`
    );
  }
  ...
  private async testConcurrentSearches(): Promise<void> {
    console.log('🔄 Testando buscas concorrentes...');
    ...
    console.log(
      `📊 Buscas concorrentes: ${totalTime.toFixed(2)}ms total, ${avgTimePerSearch.toFixed(2)}ms por busca`
    );
  }
  ...
  private async testWebSocketPerformance(): Promise<void> {
    console.log('🌐 Testando performance do WebSocket...');
    ...
    console.log(
      `📊 WebSocket: ${totalTime.toFixed(2)}ms total, ${avgTimePerMessage.toFixed(2)}ms por mensagem`
    );
  }
  ...
  private async testAlertProcessingImpact(): Promise<void> {
    console.log('🚨 Testando impacto do processamento de alertas...');
    ...
    for (let i = 0; i < alertCount; i++) {
      // Simular verificação de limites e geração de alertas
    ...
    console.log(
      `📊 Processamento de alertas: ${totalTime.toFixed(2)}ms total, ${avgTimePerAlert.toFixed(2)}ms por alerta`
    );
  }
  ...
  private getMemoryUsage(): number {
    // Em um ambiente real, usaria process.memoryUsage() no Node.js
    // ou performance.memory no browser
  ...
  private generateRecommendations(): string[] {
    ...
    if (failedTests.length === 0) {
      recommendations.push(
        '✅ Todos os testes passaram! O sistema de monitoramento tem impacto mínimo na performance.'
      );
    } else {
      failedTests.forEach(test => {
        switch (test.testName) {
          case 'Search Latency Impact':
            recommendations.push(
              '⚠️ Considere otimizar o código de monitoramento para reduzir latência de busca.'
            );
            break;
          case 'Memory Usage Impact':
            recommendations.push(
              '⚠️ Implemente limpeza automática de dados antigos para controlar uso de memória.'
            );
            break;
          case 'Concurrent Searches':
            recommendations.push(
              '⚠️ Considere implementar throttling ou pooling para buscas concorrentes.'
            );
            break;
          case 'WebSocket Performance':
            recommendations.push(
              '⚠️ Otimize o processamento de mensagens WebSocket ou implemente batching.'
            );
            break;
          case 'Alert Processing Impact':
            recommendations.push(
              '⚠️ Considere processamento assíncrono de alertas para não bloquear operações principais.'
            );
            break;
        }
      });
...
export { PerformanceTestResult, PerformanceTestSummary };
