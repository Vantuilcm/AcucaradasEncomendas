...
function checkOneSignal() {
  ...
  const oneSignalEnvAppId = process.env.EXPO_PUBLIC_ONESIGNAL_APP_ID || process.env.ONE_SIGNAL_APP_ID;
  const oneSignalEnvEnabled = process.env.EXPO_PUBLIC_ENABLE_ONESIGNAL;
  ...
  try {
    ...
    if (fileExists('app.json')) {
      ...
      const appJsonAppId = appJson?.expo?.extra?.onesignalAppId;
      if (oneSignalEnvAppId || appJsonAppId) {
        logSuccess('ID do OneSignal encontrado (EXPO_PUBLIC_ONESIGNAL_APP_ID ou app.json extra)');
      } else {
        logError('ID do OneSignal não encontrado (EXPO_PUBLIC_ONESIGNAL_APP_ID/extra)');
        oneSignalStatus = 'ERRO';
      }

      const appJsonEnable = appJson?.expo?.extra?.enableOneSignal;
      const enableFlag = oneSignalEnvEnabled === 'true' || oneSignalEnvEnabled === '1' || oneSignalEnvEnabled === 'yes' || appJsonEnable === true;
      if (enableFlag) {
        logSuccess('OneSignal habilitado (EXPO_PUBLIC_ENABLE_ONESIGNAL ou app.json extra)');
      } else {
        logWarning('OneSignal desabilitado. Defina EXPO_PUBLIC_ENABLE_ONESIGNAL=true ou extra.enableOneSignal');
