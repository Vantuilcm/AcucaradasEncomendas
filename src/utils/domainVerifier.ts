...
/**
 * Verifica se o domínio acucaradasencomendas.com.br está corretamente configurado
  ...
  try {
    ...
    if (websiteAccessible) {
      ...
      const [privacyResponse, termsResponse] = await Promise.all([
        fetch(LEGAL_DOCUMENTS.PRIVACY_POLICY, { method: 'HEAD' }).catch(() => null),
        fetch(LEGAL_DOCUMENTS.TERMS_OF_USE, { method: 'HEAD' }).catch(() => null),
      ]);
    ...
    logger.error('Erro ao verificar configuração do domínio:', error instanceof Error ? error : new Error(String(error)));
...
export const runDomainDiagnostics = async (): Promise<Record<string, any>> => {
  try {
    ...
    logger.error('Erro ao executar diagnóstico completo:', error instanceof Error ? error : new Error(String(error)));
    return {
      status: 'error',
