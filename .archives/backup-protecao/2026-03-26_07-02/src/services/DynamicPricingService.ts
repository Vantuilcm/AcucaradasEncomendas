...
export class DynamicPricingService {
  ...
  public generatePriceSuggestion(
    ...
    try {
      ...
      loggingService.error('Erro ao gerar sugestão de preço', { error, productId: product.id });
      return this.createBasicSuggestion(product, [
        {
          factor: 'erro',
          impact: 0,
  ...
  /**
   * Aplica ajustes automáticos de preço (dentro dos limites permitidos)
   * @param product Produto a ser ajustado
   * @param suggestion Sugestão de preço
   * @returns Produto com preço atualizado ou null se não houver mudança
   */
  public applyAutomaticAdjustment(
    ...
    // Verificar se o ajuste está dentro do limite para automação
    const adjustmentPercentage = Math.abs(suggestion.suggestedPrice / product.currentPrice - 1);
    if (adjustmentPercentage > this.config.maxAutoAdjustment) {
      return null; // Ajuste muito grande para ser automático
    }

    // Verificar se há confiança suficiente
