...
/**
 * Este arquivo demonstra como utilizar o DeliveryRouteService para otimizar rotas de entrega
 */
...
export function optimizeDeliveryRoutes() {
  ...
  if ((currentHour >= 7 && currentHour <= 9) || (currentHour >= 17 && currentHour <= 19)) {
    ...
    console.log('Configuração ajustada para horário normal');
  }
  ...
  // 6. Exibir resultados
  console.log(`Rotas otimizadas: ${optimizedRoutes.length}`);

  optimizedRoutes.forEach((route, index) => {
    console.log(`\nRota ${index + 1} - Entregador: ${route.driverId}`);
    console.log(`Distância total: ${route.totalDistance} km`);
    console.log(`Tempo estimado: ${route.totalTime} minutos`);
    console.log(`Horário de início: ${route.startTime}`);
    console.log(`Horário de término estimado: ${route.estimatedEndTime}`);
    console.log('Sequência de entregas:');
    ...
    route.optimizedOrder.forEach((pointId, i) => {
      ...
      if (point) {
        ...
        console.log(`     Prioridade: ${point.priority}`);
      }
    });
  });
