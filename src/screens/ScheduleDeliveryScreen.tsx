...
type ScheduleDeliveryNavigationProp = StackNavigationProp<RootStackParamList, 'ScheduleDelivery'>;
type TimeSlot = { label: string; value: string };
type DisabledDates = Record<
  string,
  {
    disabled: boolean;
...
export default function ScheduleDeliveryScreen() {
  ...
  const getDisabledDates = (): DisabledDates => {
  ...
  const filterTimeSlotsByCurrentTime = (timeSlots: TimeSlot[]) => {
  ...
  const getMarkedDates = (): DisabledDates => {
        ...
        <Card.Content>
          ...
          <View style={styles.summaryItem}>
            ...
            <Text style={styles.summaryValue}>
              R$ {cart.items.reduce((acc, i) => acc + i.price * i.quantity, 0).toFixed(2)}
            </Text>
          </View>
          {selectedDate && (
            <View style={styles.summaryItem}>
