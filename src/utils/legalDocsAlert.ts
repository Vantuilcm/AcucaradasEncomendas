...
import type { NotificationBehavior, TimeIntervalTriggerInput } from 'expo-notifications';
...
const DEFAULT_OPTIONS: AlertOptions = {
  ...
  logToConsole: true,
  logToServer: true,
  emailRecipients: ['admin@acucaradasencomendas.com.br', 'suporte@acucaradasencomendas.com.br'],
};
...
export async function sendLegalDocsAlert(
  ...
  if (options.logToConsole) {
    logger.error('Documentos Legais Indisponíveis', { body, diagnostics });
  }
...
async function sendEmailAlert({
  ...
  try {
    ...
    const response = await fetch('https://api.acucaradasencomendas.com.br/send-alert-email', {
      method: 'POST',
      headers: {
 ...
 */
async function logToServer(data: Record<string, unknown>): Promise<void> {
  try {
    ...
    */
    ...
    const response = await fetch('https://api.acucaradasencomendas.com.br/monitoring/logs', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: 'Bearer YOUR_API_KEY', // Substitua por autenticação real
    ...
    logger.error('Erro ao registrar log no servidor', error);
