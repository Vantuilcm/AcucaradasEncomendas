 ...
 * LAB: firestore-isolated — sem userRef.get/set/update (build 1295)
 */
exports.createConnectedAccount = functions
  ...
  try {
    console.log('[STRIPE_ONBOARDING_START]', {
      uid,
      email,
      role,
      build: '1295',
      mode: 'firestore-isolated',
    });
    ...
    try {
      ...
      if (Sentry && typeof Sentry.captureException === 'function') {
        Sentry.captureException(error, {
          extra: { uid, email, role, build: '1295', mode: 'firestore-isolated' },
        });
      }
    } catch (sentryError) {
...
exports.createStripeOnboardingLink = functions
  ...
  try {
    // URLs estáveis (HTTPS) — evita Safari preto ao finalizar onboarding no iOS
...
exports.syncStripeAccountStatus = functions.https.onCall(async (data, context) => {
  ...
  try {
    console.log('[STRIPE_SYNC_START]', { uid, accountId, build: '1294' });
    ...
    console.log('[STRIPE_ACCOUNT_RETRIEVED]', {
      uid,
      accountId,
      charges_enabled: account.charges_enabled,
      payouts_enabled: account.payouts_enabled,
    ...
    try {
      ...
      console.log('[STRIPE_FIRESTORE_UPDATED]', {
        uid,
        path: `users/${uid}`,
        operation: 'update',
        success: true,
      ...
      console.error('[STRIPE_FIRESTORE_UPDATE_ERROR]', {
        uid,
        path: `users/${uid}`,
        operation: 'update',
    ...
    console.error('[STRIPE_SYNC_ERROR]', {
      uid,
      accountId,
      message: error?.message,
      code: error?.code,
