...
const BackupCodesModal: React.FC<BackupCodesModalProps> = ({ visible, onClose, backupCodes }) => {
  ...
  const handleShare = async () => {
    try {
      ...
      logger.error('Erro ao compartilhar códigos:', error instanceof Error ? error : new Error(String(error)));
  ...
  const handleExportPDF = async () => {
    try {
      ...
      logger.error('Erro ao exportar PDF:', error instanceof Error ? error : new Error(String(error)));
