...
export const checkWebsiteStatus = async (): Promise<WebsiteStatusResult> => {
  try {
    ...
    const isWebsiteOnline = !!websiteResponse?.ok;
    ...
    const isPrivacyAvailable = !!privacyResponse?.ok;
    const isTermsAvailable = !!termsResponse?.ok;
    ...
    logger.error('Erro ao verificar status do site:', error instanceof Error ? error : new Error(String(error)));
    return {
      isOnline: false,
      documentsAvailable: {
        privacyPolicy: false,
