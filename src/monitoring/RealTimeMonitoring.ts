 ...
 *
 * Este mÃ³dulo implementa monitoramento em tempo real para o sistema de busca avanÃ§ada,
 * conforme recomendado nos prÃ³ximos passos do documento de otimizaÃ§Ãµes.
 *
 * Funcionalidades:
 * - Monitoramento de mÃ©tricas de performance em tempo real
...
/**
 * Interface para configuraÃ§Ã£o do monitoramento em tempo real
 */
interface RealTimeConfig {
  /** Intervalo de coleta de mÃ©tricas em milissegundos */
  metricsInterval: number;
  /** Intervalo de verificaÃ§Ã£o de alertas em milissegundos */
  alertsInterval: number;
  /** NÃºmero mÃ¡ximo de pontos de dados mantidos em memÃ³ria */
  maxDataPoints: number;
  /** Habilitar notificaÃ§Ãµes em tempo real */
...
/**
 * Interface para dados de mÃ©trica em tempo real
 */
...
/**
 * Interface para estatÃ­sticas de dashboard
 */
...
export class RealTimeMonitoring {
  ...
  private alertsInterval: ReturnType<typeof setTimeout> | null = null;
  ...
  /**
   * Construtor privado para implementar padrÃ£o Singleton
   */
  ...
  /**
   * Inicializa as mÃ©tricas em tempo real
   */
  ...
  public start(): void {
    if (this.isRunning) {
      this.logger.warn('Monitoramento em tempo real jÃ¡ estÃ¡ em execuÃ§Ã£o');
      return;
    }
    ...
    this.logger.info('Monitoramento iniciado', {
      ...
      alertsInterval: this.config.alertsInterval
    });
  }
  ...
  public stop(): void {
    if (!this.isRunning) {
      this.logger.warn('Monitoramento em tempo real nÃ£o estÃ¡ em execuÃ§Ã£o');
      return;
    }
  ...
  private collectMetrics(): void {
    try {
      ...
      // Obter estatÃ­sticas do SearchMonitoring
      const stats = this.searchMonitoring.generatePerformanceReport();

      // Coletar mÃ©tricas especÃ­ficas
      ...
      // Notificar callbacks de mÃ©tricas
  ...
  /**
   * Adiciona uma mÃ©trica ao armazenamento em tempo real
   */
  private addMetric(metricType: string, value: number, timestamp: number): void {
    ...
    // Manter apenas os Ãºltimos N pontos de dados
  ...
  /**
   * Calcula a tendÃªncia de uma mÃ©trica
   */
  private calculateTrend(
    ...
    const recentMetrics = metrics.slice(-5); // Ãšltimas 5 mediÃ§Ãµes
    const average = recentMetrics.reduce((sum, m) => sum + m.value, 0) / recentMetrics.length;

    const threshold = average * 0.05; // 5% de variaÃ§Ã£o
  ...
  /**
   * Determina a severidade de uma mÃ©trica
   */
  private determineSeverity(metricType: string, value: number): 'normal' | 'warning' | 'critical' {
    ...
    // Limites padrÃ£o baseados no tipo de mÃ©trica
    switch (metricType) {
      case 'searchLatency':
  ...
  /**
   * Verifica alertas baseados nas mÃ©tricas atuais
   */
    ...
    try {
      for (const [metricType, metrics] of this.realTimeMetrics) {
        ...
        if (latestMetric.severity === 'warning' || latestMetric.severity === 'critical') {
          ...
          // Resolver alertas existentes se a mÃ©trica voltou ao normal
  ...
  private createAlert(metricType: string, metric: RealTimeMetric): void {
    ...
    // Verificar se jÃ¡ existe um alerta ativo para esta mÃ©trica
    if (this.activeAlerts.has(alertId)) {
      return; // Alerta jÃ¡ existe
    }
    ...
    this.logger.warn(`ALERTA ${alert.severity.toUpperCase()}`, {
  ...
  private generateAlertMessage(metricType: string, metric: RealTimeMetric): string {
    ...
    switch (metricType) {
      case 'searchLatency':
        return `LatÃªncia de busca elevada: ${value}ms (limite: ${threshold}ms)`;
      case 'errorRate':
        return `Taxa de erro elevada: ${value}% (limite: ${threshold}%)`;
      case 'memoryUsage':
        return `Uso de memÃ³ria elevado: ${value}% (limite: ${threshold}%)`;
      case 'cacheHitRate':
        return `Taxa de acerto do cache baixa: ${value}% (mÃ­nimo: ${threshold}%)`;
      case 'zeroResults':
        return `Taxa de buscas sem resultado elevada: ${value}% (limite: ${threshold}%)`;
      default:
        return `MÃ©trica ${metricType} fora do limite: ${value} (limite: ${threshold})`;
    }
  }

  /**
   * ObtÃ©m o limite para uma mÃ©trica e severidade
   */
  ...
  private getCurrentMemoryUsage(): number {
    try {
      ...
      this.logger.warn('NÃ£o foi possÃ­vel obter uso de memÃ³ria', { error });
      return 0;
    }
  }
  ...
  /**
   * Registra callback para mÃ©tricas
   */
  ...
  private notifyAlertCallbacks(alert: AlertEvent): void {
    for (const callback of this.alertCallbacks) {
      try {
        ...
        this.logger.error('Erro ao notificar callback de alerta', error);
      }
    }
  }

  /**
   * Notifica callbacks de mÃ©tricas
   */
  private notifyMetricsCallbacks(): void {
    ...
    for (const callback of this.metricsCallbacks) {
      try {
        ...
        this.logger.error('Erro ao notificar callback de mÃ©tricas', error);
      }
    }
  }
  ...
  /**
   * ObtÃ©m total de buscas realizadas
   */
  ...
  public clearHistory(): void {
    ...
    this.initializeMetrics();
    this.logger.info('HistÃ³rico de monitoramento limpo');
  }
  ...
  public updateConfig(newConfig: Partial<RealTimeConfig>): void {
    ...
    this.logger.info('ConfiguraÃ§Ã£o de monitoramento atualizada', { config: this.config });
  }
  ...
  public generateHealthReport(): {
    ...
    // Avaliar saÃºde de cada mÃ©trica
    ...
    if (stats.searchLatency.current > 1000) {
      recommendations.push('Otimizar algoritmos de busca para reduzir latÃªncia');
    }

    if (stats.cacheHitRate.current < 50) {
      recommendations.push('Revisar estratÃ©gia de cache para melhorar taxa de acerto');
    }
    ...
    if (stats.memoryUsage.current > 75) {
      recommendations.push('Otimizar uso de memÃ³ria ou aumentar recursos disponÃ­veis');
    }
    ...
    const summary =
      ...
      `latÃªncia mÃ©dia: ${stats.searchLatency.average.toFixed(0)}ms, ` +
...
// Exportar tipos para uso em outros mÃ³dulos
