...
import { runMonitoringPerformanceTests } from '../src/tests/performance/monitoringPerformance.test';
...
export default function AdminMonitoringScreen() {
  ...
  const runPerformanceTests = async () => {
    ...
    try {
      const results = await runMonitoringPerformanceTests();
      setPerformanceResults(results);
      Alert.alert(
        'Testes Concluídos',
        `Executados ${results.results.length} testes.\nTempo total: ${results.totalDuration}ms\nTaxa de sucesso: ${results.successRate}%`
      );
    } catch (error) {
        ...
        {performanceResults && (
          <View style={styles.resultsCard}>
            ...
            <Text style={styles.resultText}>
              Testes executados: {performanceResults.results.length}
            </Text>
            <Text style={styles.resultText}>
              Duração total: {performanceResults.totalDuration}ms
            </Text>
            <Text style={styles.resultText}>
              Taxa de sucesso: {performanceResults.successRate}%
            </Text>
            <Text style={styles.resultText}>
              Latência média: {performanceResults.averageLatency}ms
            </Text>
          </View>
        )}
